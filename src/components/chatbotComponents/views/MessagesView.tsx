                ...
                recorder.onstop = async () => {
                    ...
                    try {
                        const uploadResponse = await fetch('https://api.assemblyai.com/v2/upload', {
                            method: 'POST',
                            headers: { 'authorization': ASSEMBLYAI_API_KEY, 'content-type': audioBlob.type },
                            body: audioBlob,
                        });
                        ...
                        const pollForTranscript = async () => {
                            try {
                                ...
                                if (pollData.status === 'completed') {
                                    ...
                                    setTimeout(pollForTranscript, 3000);
                                }
                            } catch (err) {
                                setMicError(err instanceof Error ? err.message : 'Error fetching transcription status.');
                                console.error(err);
                                setIsTranscribing(false);
                            }
                        };
                        setTimeout(pollForTranscript, 1000);
                    } catch (err) {
                        setMicError(err instanceof Error ? err.message : 'Error transcribing audio. Please try again.');
                        console.error(err);
                        setIsTranscribing(false);
                    }
                    ...
                    <div key={msg.id} className={`flex flex-col items-${msg.isUser ? 'end' : 'start'}`}>
                        <div className={`flex items-end gap-2 max-w-[90%] ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                            <div
                                ...
                                className={`max-w-full px-4 py-2 rounded-2xl text-left cursor-pointer ${msg.isUser ? 'bg-orange-500 text-white rounded-br-lg' : 'bg-white text-gray-800 rounded-bl-lg border border-gray-200'}`}
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="w-full">
                    <div className="flex items-end w-full border border-gray-300 rounded-2xl p-2 focus-within:ring-2 focus-within:ring-brand-orange transition-shadow duration-200">
                        <textarea
                            ...
                            placeholder={isTranscribing ? "Transcribing..." : isGenerating ? "Wait for response..." : "Ask a question..."}
                            className="flex-1 resize-none outline-none bg-transparent text-gray-800 text-base pb-0.5"
                            rows={1}
